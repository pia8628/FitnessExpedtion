...
class Logic:
    ...
    def draw_monsters_for_map(self, map_info: models.MapInfo, week: Optional[int]) -> List[models.Task]:
        ...
        if week is not None:
            event_codes = self._get_event_codes_for_week(str(week))
        status_done = {"?擊殺", "??失敗"}  # 已完成或已失敗的任務狀態
    ...
    def settle_week(
        ...
        progress = 0
        if map_info:
            progress = self.get_map_progress(map_info.map_id) + 1
        ...
        # 構建訊息
        message = (
            f"已結算本週：清除上週任務 {cleared} 筆、重置技能 {reset_count} 筆，"
            f"事件/怪物抽卡完成。"
        )
        # 檢查是否進入 BOSS 階段
    ...
    def complete_task(self, task: models.Task, player: models.PlayerState) -> None:
        ...
        if "FIRST_EXERCISE_EXP=1" in event_codes:
            if not self._has_team_completion(week_str, log_data, log_idx):
                base_exp = 1
        ...
        self.repo.delete_task(task.monster_id)
        self.tasks = [t for t in self.tasks if t.monster_id != task.monster_id]
        if level_log:
            self.repo.append_log(level_log)
    ...
    def _get_event_codes_for_week(self, week: Optional[str]) -> List[str]:
        ...
        if not event or not event.effect_code:
            return []
        return [c.strip() for c in event.effect_code.split(",") if c.strip()]
